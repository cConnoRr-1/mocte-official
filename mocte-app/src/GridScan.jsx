...
export default function GridScan({
  ...
  const accumulatedScanTimeRef = useRef(0)
  const scanTravelSpeedRef = useRef(scanTravelSpeed)

  scanTravelSpeedRef.current = scanTravelSpeed
  ...
  useEffect(() => {
    ...
    const uniforms = {
      ...
      uTilt: { value: 0 },
      uYaw: { value: 0 },
      uLineThickness: { value: effectiveLineThickness },
      uLinesColor: { value: srgbColor(linesColor) },
      uScanColor: { value: srgbColor(scanColor) },
      uGridScale: { value: effectiveGridScale },
      uLineStyle: { value: lineStyle === 'dashed' ? 1 : lineStyle === 'dotted' ? 2 : 0 },
      uLineJitter: { value: Math.max(0, Math.min(1, effectiveLineJitter)) },
      ...
      uScanDuration: { value: effectiveScanDuration },
    ...
    if (enablePost) {
      ...
      const chroma = new ChromaticAberrationEffect({
        offset: new THREE.Vector2(effectiveChromaticAberration, effectiveChromaticAberration),
    ...
    const tick = () => {
      ...
      accumulatedScanTimeRef.current += dt * scanTravelSpeedRef.current
      material.uniforms.uScanTime.value = accumulatedScanTimeRef.current
  ...
  useEffect(() => {
    ...
    if (m) {
      ...
      u.uScanDelay.value = Math.max(0.0, effectiveScanDelay)
    }
    if (bloomRef.current) {
